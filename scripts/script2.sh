...
for sra in $(cat "$1"); do
  ...
  if [[ ( $read2_count > 0 ) && ( $read1_count > 0 ) ]]; then
    ...
    printf "Paired End - %d magicblast create sam\n" "$read1_count"
    date
    ...
    date
    magicblast -num_threads $cores  -infmt fasta -query $outdir/${sra}_unmapped_read_one_trimmed -query_mate $outdir/${sra}_unmapped_read_two_trimmed -score 50 -penalty -3 -outfmt tabular -out $outdir/$sra.CARD_gene.out.tab -db $cardgene/cardgenedb
